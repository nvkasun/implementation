...
# ===========================================================================
# Collector-only HTTP surface (correction pass): the duplicate portal
# previously hosted in gg_monitor_core.py (collect_portal_status,
# render_portal_html, GET / and GET /api/status) has been removed entirely.
# This component exposes ONLY /healthz and /readyz -- every other path,
# including the two former portal routes, must now 404. The single
# user-facing portal is monitoring/monitor/monitor.py + helm/goldengate-monitor.
# ===========================================================================
class CollectorHttpSurfaceTests(unittest.TestCase):
    def _handler(self, ready_state=None, expected_pipelines=None):
        ready_state = ready_state if ready_state is not None else {"gg-oracle-payments-01": True}
        expected_pipelines = expected_pipelines if expected_pipelines is not None else ["gg-oracle-payments-01"]
        handler_cls = core._make_handler(ready_state, expected_pipelines)
    ...
    def test_healthz_returns_200(self):
        ...
        handler.do_GET()
        args, _ = handler._write.call_args
        self.assertEqual(args[0], 200)

    def test_readyz_returns_200_when_ready(self):
        handler = self._handler(ready_state={"gg-oracle-payments-01": True})
        handler.path = "/readyz"
        handler.do_GET()
        args, _ = handler._write.call_args
        self.assertEqual(args[0], 200)
    ...
    def test_readyz_returns_503_when_not_ready(self):
        handler = self._handler(ready_state={"gg-oracle-payments-01": False})
        handler.path = "/readyz"
        handler.do_GET()
        args, _ = handler._write.call_args
    ...
    def test_api_status_path_returns_404(self):
        handler = self._handler()
    ...
    def test_make_handler_signature_has_no_portal_parameters(self):
        """_make_handler must take only (ready_state, expected_pipelines) --
        no portal_table_factory/portal_runtimes parameter remains."""
        import inspect
        sig = inspect.signature(core._make_handler)
        self.assertEqual(list(sig.parameters), ["ready_state", "expected_pipelines"])

    def test_no_portal_code_remains_in_module(self):
        """None of the removed portal symbols exist anywhere in the module
        under test."""
        for name in ("collect_portal_status", "render_portal_html", "read_lease",
                    "query_process_states", "PORTAL_CLIENT_SAFE_ERROR",
                    "PORTAL_STALE_SECONDS", "_classify_process_status_code",
                    "_sanitized_process_error", "_STATUS_CODE_MESSAGES"):
            self.assertFalse(hasattr(core, name), f"{name} should have been removed from gg_monitor_core")

    def test_healthz_never_touches_dynamodb(self):
    ...
    def test_readyz_never_touches_dynamodb(self):
        """/readyz reflects lease-control-loop health via ready_state (an
        in-process dict), never a DynamoDB call of its own."""
        with mock.patch("boto3.resource") as mock_resource:
            handler = self._handler()
            handler.path = "/readyz"
            handler.do_GET()
    ...
    def test_lease_manager_unaffected_by_portal_removal(self):
        """The collector's own LEASE contract (acquire/renew semantics) is
        completely unaffected by the portal removal."""
        import inspect
        self.assertIn("acquire", dir(core.LeaseManager))
        self.assertIn("renew", dir(core.LeaseManager))
        src = inspect.getsource(core.LeaseManager)
        self.assertIn("ConditionExpression", src)

    @mock_aws
    def test_lease_acquisition_still_works_end_to_end(self):
        client = boto3.client("dynamodb", region_name="eu-west-1")
        client.create_table(
            TableName="gg-eks-pipeline",
            KeySchema=[{"AttributeName": "pipeline", "KeyType": "HASH"},
    ...
    @mock_aws
    def test_state_writes_unchanged(self):
        """STATE#_deployment / STATE#<process> writes are unaffected by the
        portal removal -- write_process_state/read_process_state still work
        exactly as before."""
        client = boto3.client("dynamodb", region_name="eu-west-1")
        client.create_table(
            TableName="gg-eks-pipeline",
        ...
        ok = core.write_process_state(
            table, mgr, "gg-oracle-payments-01", "oracle", "EXTORA1",
            {"status": "RUNNING", "recordedAt": 1000}, lambda: True,
        )
        self.assertTrue(ok)
...
class CloudWatchKillSwitchTests(unittest.TestCase):
    ...
    def test_healthz_readyz_do_not_require_cloudwatch(self):
        """The collector's only HTTP surface (/healthz, /readyz) has no
        CloudWatch reference and works normally regardless of the kill
        switch."""
        core.CLOUDWATCH_PUBLISH_ENABLED = False
        ready_state = {"gg-oracle-payments-01": True}
        handler_cls = core._make_handler(ready_state, ["gg-oracle-payments-01"])
        import inspect
        src = code_only(inspect.getsource(handler_cls))
        self.assertNotIn("cloudwatch", src.lower())
