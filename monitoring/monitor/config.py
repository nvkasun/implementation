"""config.py: loads the staged goldengate-deployments.yaml registry (generated by automation/goldengate-deployment-model.py, mounted read-only into the pod) and derives runtime config."""
...
# Mirrors automation/goldengate-deployment-model.py's own _INGRESS_HOST_RE / automation/goldengate-environment.py's own _DNS_DOMAIN_RE -- re-validates the registry's already-canonically-resolved ingressHost here, defensively, rather than trusting it blindly (see _is_valid_ingress_host below); never re-derives it from name+dnsDomain (that rule now lives solely in automation/goldengate-deployment-model.py).
...
def _is_literal_bool(value):
    """True only for the literal Python bool type -- mirrors automation/goldengate-deployment-model.py's own _is_literal_bool; YAML 1.1 "yes"/"no"/"true"/"false" strings and 0/1 integers must never pass (bool is a subclass of int in Python, but isinstance(1, bool) is still False -- only True/False themselves satisfy this)."""
    return isinstance(value, bool)


def _is_valid_ingress_host(value):
    """Structural re-validation of the registry's already-canonical ingressHost -- never a second hostname-derivation rule, only a shape check on the value automation/goldengate-deployment-model.py already resolved."""
...
def load_deployments(repo_root=None):
    ...
    for entry in entries:
        ...
        # ingressEnabled is a REQUIRED, literal Boolean field of the generated registry (automation/goldengate-deployment-model.py's build_registry() always sets it) -- no truthy/falsy coercion of a string/int/null is ever accepted, so a malformed value such as "false" (a truthy Python string) can never be silently treated as enabled.
        ...
        # ingressHost is likewise REQUIRED and must already be a validly-shaped hostname -- the registry's own canonically-resolved value (see automation/goldengate-deployment-model.py's runtimeIngressHost), never reconstructed here from name+dnsDomain. Malformed canonical topology fails the whole config load closed rather than silently producing no link for one runtime.
