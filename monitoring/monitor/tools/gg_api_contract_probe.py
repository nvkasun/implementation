"""Standalone, read-only, operator-invoked-only GoldenGate Admin/Metrics REST contract-probe tool (never run automatically, not imported/exposed elsewhere; prints sanitized STRUCTURAL metadata only, never a raw value/credential/URL, and never writes DynamoDB or CloudWatch): use --port admin (HTTPS, authenticated) for the CONFIRMED secure PMS routes /services/v2/mpoints/processes and /services/v2/monitoring/statusChanges; --port metrics issues plain HTTP, unauthenticated only, since metricsPort 9015 is confirmed plain http and not an approved authenticated path (no automatic HTTPS<->HTTP fallback -- the scheme is a fixed function of --port); /services/v2/metrics is confirmed invalid (HTTP 404) and is not the production pms endpoint, so it must never be used as a recommended example; --follow-processes GETs the confirmed process inventory once, then issues up to MAX_FOLLOWED_PROCESSES sequential, bounded --detail GETs over --port admin, never outputting a process name/ID/URL."""
...
# Oversized body -> fixed, sanitized UNEXPECTED_RESPONSE, never parsed/sized/echoed; not operator-tunable.
...
class ProbeValidationError(Exception):
    """A local pre-flight validation failure (bad args, unknown/disabled deployment, unsafe path) -- never an HTTP-request-outcome category."""


class ProbeRequestError(Exception):
    """A classified HTTP/TLS/network outcome; category is always one of ERROR_CATEGORIES, http_status the raw status code where one exists."""
...
def _reject_unsafe_decoded_form(path, decoded):
    """Rejects a decoded round that introduces a new backslash, control character, percent-encoded '/', or '.'/'..' segment -- percent-encoding must never smuggle what the literal path forbids."""
...
def _reject_unsafe_percent_encoding(path, max_rounds=5):
    """Bounded, iterative percent-decoding (guards against double-encoding evasion, e.g. %252e%252e); stops once decoding stabilizes, raises if the safe-decode depth is exceeded."""
    current = path
...
def validate_path(path):
    """Accepts only a bare, already-normalized /services/... path -- never a URL, scheme, host, query/fragment, backslash, control character, or literal/percent-encoded traversal; no broad allowlist, so any other legitimate /services/... endpoint remains probeable."""
    ...
    # A canonical path is already normalized; any difference (duplicate slashes, "." / "..", trailing slash) is rejected rather than silently resolved.
...
def resolve_deployment(name, repo_config_root=None):
    """Loads the canonical deployment list and returns the one matching name; rejects an unknown or disabled deployment."""
...
def _port_and_base(deployment, port_type):
    """admin -> HTTPS through adminPort 8443 (confirmed authenticated PMS route); metrics -> plain HTTP through metricsPort 9015 (confirmed plain HTTP live, never credentialed); scheme is a fixed function of port_type, no automatic HTTPS<->HTTP fallback."""
...
def _contains_tls_error(exc, max_nodes=10):
    """Bounded, cycle-safe search for an ssl.SSLError anywhere in exc's chain (itself, URLError.reason, __cause__/__context__ at every node); never returns or logs the exception text -- classification only."""
    if exc is None:
        return False
    seen_ids = set()
    stack = [exc]
...
# Bounds collection inspection so a large/hostile PMS payload can't consume unbounded memory/output; itemCount always reports the TRUE list length, only per-item/per-field-name inspection is capped.
MAX_COLLECTION_KEYS = 20
MAX_ITEMS_PER_COLLECTION = 50
MAX_FIELD_NAMES_PER_COLLECTION = 100

# Bounds key-name OUTPUT; a key longer than MAX_KEY_LENGTH is omitted entirely (never partial) and the truncated flag is set.
...
def _bounded_sorted_keys(keys, max_count, max_key_length=MAX_KEY_LENGTH):
    """Sorted, deterministic key list for direct output: a key longer than max_key_length is dropped entirely (never partial), then capped to max_count; returns (keys, truncated)."""
...
def _summarize_collection(items, max_items=MAX_ITEMS_PER_COLLECTION,
    ...
    """items is a confirmed list; returns sanitized structural metadata only (field NAMES and broad JSON TYPES, never a raw value). Non-dict members are skipped; nested objects/arrays are reported only as "object"/"array", never recursed into."""
...
def summarize_json(payload, max_items=MAX_ITEMS_PER_COLLECTION,
                   ...
                   max_field_names=MAX_FIELD_NAMES_PER_COLLECTION,
                   max_key_length=MAX_KEY_LENGTH):
    """Sanitized structural metadata only (collection field NAMES and broad JSON TYPES, never a raw value); returns None when payload is not a top-level JSON object. Every list-valued field directly under response.* becomes its own "collections" entry (not just response.items), dropped deterministically by sorted field name once max_collection_keys is exceeded."""
    ...
    if isinstance(response, dict):
        ...
        # A collection name longer than max_key_length is omitted entirely (never partial); its items are never inspected.
    ...
    # Legacy flat fields, kept for backward compatibility only when response.items exists -- never mapped from a different collection.
...
def _fetch_json(deployment, port_type, path, timeout=PROBE_TIMEOUT_SECONDS):
    """Performs exactly one read-only GET for path, returning (http_status, content_type, payload) or raising ProbeRequestError (closed category) on any failure; never returns/logs a raw body, exception text, header, or URL. Shared by run_probe and --follow-processes. port_type="admin" is HTTPS/adminPort 8443, authenticated with the collector's CSI-mounted credentials; port_type="metrics" is plain HTTP/metricsPort 9015, always unauthenticated."""
    ...
    if port_type == "admin":
        ...
        opener = urllib.request.build_opener()  # plain HTTP, unauthenticated -- never build an auth handler or read a credential file
    ...
    try:
        with opener.open(url, timeout=timeout) as resp:
            ...
            # Read at most one byte past the limit to detect an oversized body without reading it unboundedly.
    ...
    if len(raw_body) > MAX_RESPONSE_BYTES:
        raise ProbeRequestError("UNEXPECTED_RESPONSE", http_status=http_status)  # oversized: never parsed, sized, or echoed
...
def run_probe(deployment, port_type, path, timeout=PROBE_TIMEOUT_SECONDS):
    """Performs exactly one read-only GET, returning a sanitized result dict on success; raises ProbeRequestError (closed category) on failure and never returns/logs a raw body, exception text, header, or URL."""
...
# --follow-processes: fixed detail-endpoint allowlist -- never an arbitrary operator-supplied suffix.
...
def _valid_inventory_process_names(payload):
    """Returns (names, inventory_item_count): unique processName strings from response.processes, in first-seen order (never processId); a duplicate name must never cause a second detail request."""
    if not isinstance(payload, dict):
        return [], 0
    response = payload.get("response")
...
def _process_detail_path(name, detail):
    """Encodes name as exactly one URL path segment: quote(name, safe="") can never introduce a new segment, but RFC 3986 leaves '.'/'..' unescaped, so those two exact values are rejected outright as traversal-equivalent (validate_path's percent-slash rejection is not reused here -- it guards a different, --path-smuggling threat)."""
    if name in (".", ".."):
...
def _merge_detail_schema(payload, agg):
    """Merges one successful detail response's structural schema into the running aggregate (agg); payload is discarded by the caller right after -- no response body is retained past schema merging."""
...
def _finalize_schema(agg):
    """Bounds the merged aggregate one final time (merging many responses could otherwise still exceed the per-response limits) and renders it into the sanitized output shape."""
    ...
    # A merged collection NAME longer than MAX_KEY_LENGTH is omitted entirely (never partial); its data is never inspected/output.
    ...
    for name in collection_names:
        ...
        # Re-apply per-collection field-name bounds: merging many responses' field names can exceed the cap even though each individual response stayed within it.
...
def follow_processes(deployment, detail, timeout=PROBE_TIMEOUT_SECONDS,
    ...
    """--follow-processes mode: GETs the confirmed process inventory then issues up to max_followed sequential, bounded detail GETs (one per valid processName, never processId), merging their structural schemas; never outputs a process name, ID, or constructed URL -- only counts and merged schema. One failed detail request never stops the rest; always returns a result dict (even if every detail request failed) so the caller can still see aggregate error-category counts and decide the exit code from successCount."""
