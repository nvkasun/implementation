"""Offline tests for hack/orchestration/monitor_state.py; run directly via `python3 hack/test-goldengate-monitor-state.py`. No live Kubernetes -- every kubectl response is a fake, injected fixture. Exercises the classifier's actual logic (never merely greps its source)."""
...
TOOL_PATH = os.path.join(REPO_ROOT, "hack", "orchestration", "monitor_state.py")
