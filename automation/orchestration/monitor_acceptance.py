#!/usr/bin/env python3
"""automation/orchestration/monitor_acceptance.py: read-only GoldenGate shared monitor post-reconciliation acceptance classifier (Phase B3B) -- answers exactly one question, "is the shared gg-monitor deployment fully healthy right now, and does it see the SAME canonical registry as the current global active runtime inventory?", as one of HEALTHY/BROKEN. Unlike automation/orchestration/monitor_state.py (a pre-reconciliation ownership-safety preflight), this tool DOES require full readiness/health: a monitor that is missing, unhealthy, or shaped incorrectly after reconciliation is BROKEN. Never mutates the cluster: every kubectl invocation here is a `get`/`get -l` (read-only); no apply/create/delete/patch/annotate/label/helm call exists in this module, and it never performs an HTTP request itself -- the actual /healthz and /readyz in-pod HTTP checks are performed by the calling workflow (bounded kubectl exec, read-only) against the exact Ready pod this tool selects and returns; their results are only OPTIONALLY fed back in via --healthz-status/--readyz-status for a second, final pass. The canonical registry document (automation/goldengate-deployment-model.py registry) is always passed in as an already-generated local file -- this tool never independently rebuilds it."""
...
def _check_canonical_configmap(run, reasons, monitor_namespace, registry):
    ...
    if actual_doc != registry:
        reasons.append(
            ...
            "(automation/goldengate-deployment-model.py registry) -- the monitor may be serving a stale, missing, or extra deployment"
        )
...
def main(argv=None):
    ...
    parser.add_argument("--registry-file", required=True, help="Path to a locally-generated `automation/goldengate-deployment-model.py registry` YAML/JSON document -- never rebuilt independently by this tool.")
