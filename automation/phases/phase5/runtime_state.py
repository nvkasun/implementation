...
def classify(run, environment, deployment_id, argocd_namespace, runtime_namespace, ecr_registry, retained_pvc_expected=False):
    ...
    # GoldenGate Runtime Presence Contract -- Final Safety Correction, Gap 5: the u02 PVC is intentionally retained across Application deletion (Prune=false, see helm/goldengate/templates/runtime-pvc.yaml) -- "Application absent, ONLY the retained PVC exists" is the expected, SAFE shape of a disabled-then-re-enableable runtime, never an unexplained orphan on its own. Every OTHER compute/workload footprint kind (StatefulSet/Service/headless Service/StorageClass/SecretProviderClasses/admin Secret) is still pruned/cascade-deleted as normal and remains exactly as unsafe as before when found without an owning Application AND without an owning ApplicationSet. "Chart-owned" persistence means the descriptor both declares EFS persistence (efsMode is not None) AND the chart actually creates/owns this PVC (descriptor.get("u02ChartOwnsPvc"), automation/goldengate-deployment-model.py's own canonical field -- false ONLY when runtime.storage.u02.existingClaim is set; a non-empty custom runtime.storage.u02.claimName is STILL chart-owned, distinguishing Chart-Owned vs External u02 Claim correctly rather than treating any non-empty override as external) -- the SAME condition helm/goldengate/templates/runtime-pvc.yaml itself renders on.
    declares_chart_owned_persistence = bool(descriptor and descriptor.get("efsMode") and descriptor.get("u02ChartOwnsPvc"))
