...
if [ "$EVENT_NAME" = "workflow_dispatch" ]; then
  ...
  if [ -z "$DEPLOYMENT_ID" ]; then
    ...
    ENV_MATRIX_JSON="$(python3 automation/goldengate-deployment-model.py --environment "$ENVIRONMENT" environment-matrix --deploy "$DEPLOY")"
