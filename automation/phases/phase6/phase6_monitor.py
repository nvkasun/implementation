#!/usr/bin/env python3
"""Phase 6A-6E monitor orchestration used by 60-phase-monitor-final-acceptance.yaml. Invokes the canonical environment/deployment CLIs and existing ownership, monitor-health, and end-to-end classifiers as subprocesses. monitor_sync_once remains a separate reusable workflow call. Credentials are never logged or written to outputs/state files; API failure diagnostics are bounded and sanitized. GoldenGate replication is configured manually through the UI. Acceptance checks passive monitor/runtime health and generic process safety, never desired-process-name conformance."""
