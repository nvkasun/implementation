...
class Phase6MonitorError(Exception):
    """A fail-closed Phase 6 monitor orchestration error; main() reports it and exits non-zero."""


def require_env(name):
    value = os.environ.get(name, "")
    if not value:
        raise Phase6MonitorError(f"{name} is empty; canonical environment configuration must be loaded before this step.")
    return value
...
def require_environment_arg(environment):
    ...
    if not isinstance(environment, str) or not environment:
        raise Phase6MonitorError(f"environment {environment!r} is not a usable identifier; refusing to use it.")
    return environment


def require_pod_name_arg(pod_name):
    if not pod_name:
        raise Phase6MonitorError("no verified Ready gg-monitor pod name was provided -- refusing to proceed without a pod already selected/health-checked by strict-acceptance.")
    return pod_name
...
def run(argv, env=None, cwd=None, check=True, capture_output=True, input_text=None, timeout_seconds=None):
    ...
    if check and proc.returncode != 0:
        raise Phase6MonitorError(f"{' '.join(str(a) for a in argv)} failed (exit {proc.returncode}):\n{proc.stdout}\n{proc.stderr}")
    return proc
...
def _ensure_kubectl():
    ...
    if machine not in arch_map:
        raise Phase6MonitorError(f"Unsupported architecture for kubectl: {machine}")
...
# Phase 6A: monitor ownership preflight (monitor_ownership_preflight) -- reuses automation/orchestration/monitor_state.py, never a second ownership classifier.
...
def cmd_ownership_preflight(args):
    ...
    if proc.returncode != 0:
        raise Phase6MonitorError("the GoldenGate monitor ownership classifier could not classify the shared monitor (configuration or inspection error, not ABSENT) -- see diagnostics above.")

    _print_proc_stdout(proc)
    try:
        result = json.loads(proc.stdout)
    except json.JSONDecodeError as exc:
        raise Phase6MonitorError(f"the GoldenGate monitor ownership classifier produced unparseable output: {exc}") from exc

    state = result.get("state")
    if state not in ("ABSENT", "OWNED", "BROKEN"):
        raise Phase6MonitorError(f"the GoldenGate monitor ownership classifier produced an unrecognized or missing state {state!r}; refusing to proceed.")

    print(f"GoldenGate monitor ownership-safety state: {state}")
    write_github_output([("state", state)])

    if state == "BROKEN":
        raise Phase6MonitorError("GoldenGate monitor ownership-safety state is BROKEN -- an existing footprint does not clearly belong to the shared monitor. This is not auto-repaired here -- investigate the diagnostics above before re-running.")


# Phase 6C: local monitor validation (monitor_dry_run_validation) -- deploy=false only, zero AWS/kubectl/mutation, private tempdir for all staged/rendered output (test evidence only, never desired state).
...
def cmd_validate_local(args):
    ...
    if run(["bash", "-c", "command -v helm"], check=False).returncode != 0:
        raise Phase6MonitorError("helm is required for local monitor chart validation and is not available on this runner.")

    with tempfile.TemporaryDirectory(prefix="phase6-monitor-dry-run-") as tmp:
...
def _kubectl_exec_http_status(pod_name, namespace, path, timeout_seconds):
    ...
    if proc.returncode != 0:
        raise Phase6MonitorError(f"kubectl exec into pod {pod_name} in namespace {namespace} for GET {path} failed (exit {proc.returncode}); sanitized diagnostic: {_sanitize_tail(proc.stderr)}")
    lines = [line for line in proc.stdout.splitlines() if line.strip() != ""]
    if len(lines) != 1:
        raise Phase6MonitorError(f"kubectl exec into pod {pod_name} for GET {path} returned {len(lines)} output line(s), expected exactly 1 (malformed health-check output).")
    try:
        return int(lines[0].strip())
    except ValueError:
        raise Phase6MonitorError(f"kubectl exec into pod {pod_name} for GET {path} returned non-integer output {lines[0]!r} (malformed health-check output).")


def _try_fetch_api_processes(pod_name, namespace, timeout_seconds=5):
    """Bounded, read-only fetch of GET /api/processes through pod_name (kubectl exec). Returns (True, parsed_json_doc) on success, or (False, sanitized_error_text) on ANY failure (kubectl exec failure, non-JSON output) -- never raises, so a bounded-retry caller can distinguish 'transient fetch failure, keep polling' from a hard failure elsewhere. The sanitized_error_text is bounded/truncated -- never a raw unbounded kubectl/API server diagnostic. The OUTER kubectl exec call itself is wall-clock bounded (timeout_seconds + _KUBECTL_EXEC_OUTER_TIMEOUT_BUFFER_SECONDS) -- a stall before the in-pod urllib timeout is ever reached (API/auth/SPDY negotiation, kubectl itself hanging) surfaces as an ordinary non-zero kubectl exec failure here, never an unbounded hang or a bare traceback. Used exclusively by end_to_end_acceptance's own bounded-retry poll below."""
...
def cmd_strict_acceptance(args):
    ...
    with tempfile.TemporaryDirectory(prefix="phase6-monitor-acceptance-") as tmp:
        ...
        if structural_proc.returncode != 0:
            raise Phase6MonitorError("the GoldenGate monitor acceptance classifier could not evaluate the shared monitor (configuration or inspection error) -- see diagnostics above.")
        _print_proc_stdout(structural_proc)
        try:
            structural_result = json.loads(structural_proc.stdout)
        except json.JSONDecodeError as exc:
            raise Phase6MonitorError(f"the GoldenGate monitor acceptance classifier produced unparseable output: {exc}") from exc
        ...
        if structural_state != "HEALTHY":
            raise Phase6MonitorError(f"GoldenGate monitor structural acceptance state is {structural_state}, expected HEALTHY. Never proceeding to /healthz or /readyz checks against an unverified/absent pod.")

        pod_name = (structural_result.get("checks") or {}).get("ready_pod_name")
        if not pod_name:
            raise Phase6MonitorError("no verified Ready pod name was returned by structural acceptance.")
        ...
        if final_proc.returncode != 0:
            raise Phase6MonitorError("the GoldenGate monitor acceptance classifier could not evaluate the shared monitor on the final pass (configuration or inspection error) -- see diagnostics above.")
        _print_proc_stdout(final_proc)
        try:
            final_result = json.loads(final_proc.stdout)
        except json.JSONDecodeError as exc:
            raise Phase6MonitorError(f"the GoldenGate monitor acceptance classifier (final pass) produced unparseable output: {exc}") from exc

        final_state = final_result.get("state")
        print(f"GoldenGate monitor final acceptance state: {final_state}")
        if final_state != "HEALTHY":
            raise Phase6MonitorError(f"GoldenGate monitor final acceptance state is {final_state}, expected HEALTHY. MAIN cannot claim the monitor is ready while it is not fully healthy (structural shape, canonical registry equality, /healthz, and /readyz all gate this).")
...
# Phase 6E: monitor-to-runtime end-to-end acceptance (end_to_end_deployment_acceptance) -- bounded poll, reuses automation/orchestration/end_to_end_acceptance.py as the sole authoritative classifier, reuses the SAME verified Ready pod. Its classifier proves monitor/runtime health and GENERIC process safety (stale/ABENDED/malformed rows fail closed) -- never desired-process conformance; a manually-unconfigured runtime (EMPTY discovery) and a manually-configured healthy runtime (OK discovery with healthy rows) are both accepted.
...
def _require_positive_int(label, value):
    ...
    if isinstance(value, bool) or not isinstance(value, int) or value <= 0:
        raise Phase6MonitorError(f"{label} must be a positive integer, got {value!r}.")
    return value
...
def cmd_end_to_end_acceptance(args):
    ...
    while True:
        ...
        if elapsed >= timeout_seconds:
            raise Phase6MonitorError(f"timed out after {timeout_seconds}s waiting for GoldenGate monitor-to-runtime end-to-end acceptance to become HEALTHY. This is never suppressed -- a persistent problem must surface as a failed workflow.")
...
def build_parser():
    parser = argparse.ArgumentParser(description="Phase 6 | GoldenGate monitor orchestrator (ownership preflight, local dry-run validation, strict acceptance, bounded end-to-end acceptance).")
...
def main(argv=None):
    ...
    try:
        args.func(args)
    except Phase6MonitorError as exc:
        print(f"FAIL: {exc}")
        return 1
