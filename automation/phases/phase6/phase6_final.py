#!/usr/bin/env python3
"""Phase 6F mode-aware final DEPLOY success gate. The workflow supplies plain environment strings for mode flags and individual job results. Literal-boolean validation, require_success(), and allow_non_failure() fail closed for each mode. Retired automated replication results are excluded from RESULT_JOB_NAMES and every mode branch: replication is configured manually through the GoldenGate UI, while runtime, passive monitor, and end-to-end acceptance establish the deployment success contract."""
