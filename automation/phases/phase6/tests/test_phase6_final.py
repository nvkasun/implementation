"""Offline tests for automation/phases/phase6/phase6_final.py; run directly via `python3 automation/phases/phase6/tests/test_phase6_final.py`. Pure-function tests only -- validate_gate() takes a plain str->str mapping and returns (ok, log), so this suite never touches a subprocess, the filesystem, or GitHub Actions itself. Covers the complete mode-aware truth table this module moved out of .github/workflows/00-main-goldengate-orchestrator.yaml's inline shell: literal-boolean-only handling, require_success()/allow_non_failure() semantics, and every effective_deploy/has_active_deployments/has_changes/has_deletions branch combination. Automated Replication Implementation Removal: this file was previously automation/phases/phase7/tests/test_phase7_final.py -- renumbered to Phase 6, and every test referencing the now-removed replication_reconcile_once/replication_dry_run_validation/replication_monitor_acceptance results is retired outright (not carried forward as skipped/dead)."""
