...
TOOL_PATH = REPO_ROOT / "automation" / "phases" / "phase6" / "phase6_monitor.py"
...
phase6_monitor = _load_tool(TOOL_PATH, "phase6_monitor")
...
class ScriptedRun:
    """Replaces phase6_monitor.run with a scripted responder: a list of (predicate, FakeProc-or-callable) pairs consulted in order (later registrations take precedence), falling back to a default success. Every call is recorded for assertion."""
        ...
        for predicate, proc_or_fn in reversed(self.rules):
            if predicate(argv):
                ...
                if check and proc.returncode != 0:
                    raise phase6_monitor.Phase6MonitorError(f"{' '.join(str(a) for a in argv)} failed: {proc.stdout}\n{proc.stderr}")
                return proc
        if check and self.default.returncode != 0:
            raise phase6_monitor.Phase6MonitorError(f"{' '.join(str(a) for a in argv)} failed: {self.default.stdout}\n{self.default.stderr}")
        return self.default
...
def _is_monitor_state_call(argv):
    return _is_tool_call(phase6_monitor.MONITOR_STATE_TOOL)(argv)


def _is_monitor_acceptance_call(argv):
    return _is_tool_call(phase6_monitor.MONITOR_ACCEPTANCE_TOOL)(argv)


def _is_end_to_end_call(argv):
    return _is_tool_call(phase6_monitor.END_TO_END_ACCEPTANCE_TOOL)(argv)


def _is_registry_call(argv):
    return _is_tool_call(phase6_monitor.DEPLOYMENT_MODEL_TOOL)(argv) and "registry" in argv
...
class EnsureKubectlTests(unittest.TestCase):
    def test_kubectl_already_present_skips_download(self):
        ...
        with mock.patch.object(phase6_monitor, "run", scripted):
            phase6_monitor.cmd_ensure_kubectl(argparse_namespace())
...
class OwnershipPreflightTests(unittest.TestCase):
    ...
    def _run(self, scripted, github_output_path):
        ...
        with _env_patch({"GITHUB_OUTPUT": github_output_path}), mock.patch.object(phase6_monitor, "run", scripted):
            return phase6_monitor.cmd_ownership_preflight(args)
...
class StrictAcceptanceTests(unittest.TestCase):
    ...
    def _run(self, scripted, github_output_path):
        ...
        with _env_patch({"GITHUB_OUTPUT": github_output_path}), mock.patch.object(phase6_monitor, "run", scripted):
            return phase6_monitor.cmd_strict_acceptance(args)
    ...
    # J: a kubectl exec WALL-CLOCK TIMEOUT on /healthz or /readyz (simulating Kubernetes API/auth/SPDY negotiation stalling before the in-pod urllib timeout is ever reached) must fail closed with a controlled Phase6MonitorError -- never an uncontrolled subprocess.TimeoutExpired traceback, never treated as HTTP 0 success.
    def test_kubectl_health_exec_timeout_fails_closed_without_traceback(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            try:
                self._run(scripted, output_path)
                self.fail("expected Phase6MonitorError")
            except phase6_monitor.Phase6MonitorError as exc:
...
class EndToEndAcceptanceTests(unittest.TestCase):
    ...
    def test_accepts_healthy_immediately(self):
        ...
        with _env_patch(), mock.patch.object(phase6_monitor, "run", scripted), mock.patch.object(phase6_monitor.time, "sleep") as sleep_mock:
            phase6_monitor.cmd_end_to_end_acceptance(args)
    ...
    def test_no_unbounded_retry_call_count_matches_bound_exactly(self):
        ...
        with _env_patch(), mock.patch.object(phase6_monitor, "run", scripted), mock.patch.object(phase6_monitor.time, "sleep"):
            with self.assertRaises(phase6_monitor.Phase6MonitorError):
                phase6_monitor.cmd_end_to_end_acceptance(args)
    ...
    def test_connects_to_eks_before_first_fetch_exactly_once(self):
        ...
        scripted.when(_is_end_to_end_call, FakeProc(0, json.dumps({"state": "HEALTHY"})))
        args = argparse_namespace(environment=ENVIRONMENT, pod_name="gg-monitor-x", timeout_seconds=600, interval_seconds=15)
        with _env_patch(), mock.patch.object(phase6_monitor, "_connect_to_eks", fake_connect), \
                mock.patch.object(phase6_monitor, "_try_fetch_api_processes", fake_fetch), \
                mock.patch.object(phase6_monitor, "run", scripted), \
                mock.patch.object(phase6_monitor.time, "sleep"):
            phase6_monitor.cmd_end_to_end_acceptance(args)
    ...
    def test_eks_connection_failure_stops_before_any_polling(self):
        def fake_connect():
            raise phase6_monitor.Phase6MonitorError("simulated EKS connection failure (AccessDenied/expired credentials/cluster not found/network failure)")

        fetch_mock = mock.Mock()
        run_mock = mock.Mock()
        args = argparse_namespace(environment=ENVIRONMENT, pod_name="gg-monitor-x", timeout_seconds=600, interval_seconds=15)
        with _env_patch(), mock.patch.object(phase6_monitor, "_connect_to_eks", fake_connect), \
                mock.patch.object(phase6_monitor, "_try_fetch_api_processes", fetch_mock), \
                mock.patch.object(phase6_monitor, "run", run_mock), \
                mock.patch.object(phase6_monitor.time, "sleep") as sleep_mock:
            with self.assertRaises(phase6_monitor.Phase6MonitorError):
                phase6_monitor.cmd_end_to_end_acceptance(args)
    ...
    def test_transient_fetch_retry_does_not_reconnect_to_eks(self):
        ...
        scripted.when(_is_end_to_end_call, FakeProc(0, json.dumps({"state": "HEALTHY"})))
        args = argparse_namespace(environment=ENVIRONMENT, pod_name="gg-monitor-x", timeout_seconds=600, interval_seconds=15)
        with _env_patch(), mock.patch.object(phase6_monitor, "_connect_to_eks", connect_mock), \
                mock.patch.object(phase6_monitor, "run", scripted), \
                mock.patch.object(phase6_monitor.time, "sleep"):
            phase6_monitor.cmd_end_to_end_acceptance(args)
    ...
    def test_e2e_connect_uses_canonical_eks_command_and_values(self):
        ...
        import inspect
        source = inspect.getsource(phase6_monitor.cmd_end_to_end_acceptance)
...
class EndToEndAcceptanceRealClassifierIntegrationTests(unittest.TestCase):
    ...
    def test_real_classifier_broken_result_can_never_be_reported_healthy(self):
        ...
        with _env_patch(), mock.patch.object(phase6_monitor, "_connect_to_eks", mock.Mock()), \
                mock.patch.object(phase6_monitor, "_try_fetch_api_processes", fake_fetch), \
                mock.patch.object(phase6_monitor.time, "sleep"), \
...
class SanitizedDiagnosticTests(unittest.TestCase):
    ...
    def test_sanitize_tail_bounds_length(self):
        huge = "x" * 5000
        sanitized = phase6_monitor._sanitize_tail(huge)
        self.assertLessEqual(len(sanitized), phase6_monitor._SANITIZED_ERROR_TAIL_CHARS)

    def test_sanitize_tail_handles_empty(self):
        self.assertEqual(phase6_monitor._sanitize_tail(""), "<no diagnostic output>")
